...
public class TimeBaseKVStore {
    ...
    public static class TimeMap {
        ...
        public String get(String key, int timestamp) {
            return Optional.ofNullable(map.get(key))
                .flatMap(it -> Optional.ofNullable(it.floorEntry(timestamp)))
                .map(it -> it.getValue())
                .orElse("");
        }
    } 
    ...
    void test(String commands, String expected) {
        var cArr = ArrayUtils.toArray(commands, String.class);
        var eArr = ArrayUtils.toArray(expected, String.class);
        var map = new TimeMap();
        map.set("love", "high", 10);
        map.set("love", "low", 20);
        var result = map.get("kk", 15);
